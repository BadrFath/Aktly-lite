...
function extractCompanyDenominationFromHtml(html) {
  ...
  const rowMatch = source.match(
    /<td[^>]*>\s*(?:D[Ã©e]nomination|Denomination|Benaming|Naam)\s*:?\s*<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>/i,
  );
  if (rowMatch?.[1]) {
...
function parseBcePublicDirigeantsFromHtml(html, enterpriseNumber) {
  ...
  const rows = [];
  const rolePattern = /(administrateur|bestuurder|g[Ã©e]rant|zaakvoerder|mandataire|gedelegeerd|directeur)/i;
...
/**
 * Generate the publication text (procÃ¨s-verbal) in plain text.
...
function generatePubTextHtml(data) {
  ...
  if (hasResolutions) {
    lines.push(isNl ? "Besluiten:" : "RÃ©solutions:");
    pushBlank();
  }
    ...
    normalizedCessions.forEach((cession) => {
      lines.push(
        isNl
          ...
          : `- ${cession.cedant} cÃ¨de ${cession.quantite} actions Ã  ${cession.acquereur}, lequel/laquelle accepte.`
      );
    });
    pushBlank();
  }
    ...
    const actionnaireLines = actionnaires
      .map((actionnaire) => {
        ...
        return isNl
          ...
          : `${nom} : ${avant} actions avant modification, ${apres} actions aprÃ¨s modification.`;
      })
      .filter(Boolean)
      .join(" ");
    ...
    if (actionnaireLines) {
      lines.push(
        isNl
          ...
          : `La rÃ©partition des actions avant et aprÃ¨s modification, pour un total de ${totalActions || capitalTotal} actions, est la suivante : ${actionnaireLines}`
      );
      pushBlank();
    }
  }
  ...
  if (isNl) {
    ...
    lines.push(
      heureFin
        ? `L'ordre du jour Ã©tant Ã©puisÃ©, l'assemblÃ©e est levÃ©e Ã  ${heureFin}.`
        : "L'ordre du jour Ã©tant Ã©puisÃ©, l'assemblÃ©e est levÃ©e."
    );
  }
...
async function buildFormulaire1HtmlPage(data, autoprint = false) {
  ...
  // Format enterprise number: 9 digits â†’ prepend leading 0
  ...
  html = html.replaceAll("__PHRASE__", phrase); // raw â€“ may contain <br>
...
async function buildFormulaire2HtmlPage(data, autoprint = false) {
  ...
  html = html.replace(/NÂ°\s*:\s*__TOKEN__/, `NÂ° :   ${branchHouseNumber}`);
  html = html.replace(/BoÃ®te\s*<span class="ls3">:\s*__TOKEN__/, `BoÃ®te <span class="ls3">:  ${branchBox}`);
  html = html.replace(/postal\s*:\s*__TOKEN__/, `postal :  ${branchZipcode}`);
  html = html.replace(/calitÃ©\s*:\s*__TOKEN__/, `calitÃ© :  ${branchMunicipality}`);
...
async function buildAttestation1HtmlPage(data, autoprint = false) {
  ...
  const fallbackName = "â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦â€¦";
...
function getTemplatePlacements(documentKey, payload) {
  ...
  const byDocument = {
    "formulaire1entr": [
      // Page 0 â€” Volet A/B identification section
      ...
      // Page 1 â€” Volet B publication copy (Moniteur belge)
...
const MONTHS_FR = ['janvier','\u00e9vrier','mars','avril','mai','juin','juillet','ao\u00fbt','septembre','octobre','novembre','d\u00e9cembre'];
function formatDateFr(dateStr) {
  if (!dateStr) return '';
...
function buildExtractDesc(obj, lang = 'fr') {
  if (!obj) return '';
  if (typeof obj === 'string') return obj;
  if (Array.isArray(obj)) {
    const found = obj.find(x => x && x.language === lang);
    if (found) return buildExtractDesc(found, lang);
    if (obj[0]) return buildExtractDesc(obj[0], lang);
    return '';
  }
  if (typeof obj === 'object') {
    if (obj.value !== undefined) return buildExtractDesc(obj.value, lang);
    if (obj.description !== undefined) return buildExtractDesc(obj.description, lang);
  }
  return String(obj);
}

function parseBceForPv(bceData) {
  if (!bceData) return {};
  const lang = bceData.lang_entre || 'fr';
  let denomination = '';
  try {
    const denomArr = bceData.denomination;
    if (Array.isArray(denomArr) && denomArr[0] && denomArr[0].description) {
      denomination = buildExtractDesc(denomArr[0].description, lang);
    } else if (typeof denomArr === 'string') {
  ...
  let dirigeants = [];
  try {
    const funcs = bceData.function || bceData.functions || [];
    if (Array.isArray(funcs)) {
      dirigeants = funcs.map(f => ({
        givenName: (f.person && f.person.givenName) || (f.data && f.data.person && f.data.person.firstName) || '',
        surname: (f.person && f.person.surname) || (f.data && f.data.person && f.data.person.lastName) || '',
        idNumber: (f.data && f.data.person && f.data.person.idNumber) || '',
        func: f.function || 'Administrateur',
      }));
    }
  } catch(e){}
...
function buildDeclarationHtml(demande) {
  ...
  function dRow(d) {
    const name = [d.givenName, d.surname].filter(Boolean).join(' ');
    return { name: name || '', id: d.idNumber || '', func: d.func || '' };
  }
  const r0 = dRow(d0), r1 = dRow(d1), r2 = dRow(d2);
  const addressStr = [bce.street, bce.houseNumber, bce.municipality, bce.zipcode].filter(Boolean).join(' ');
  const templatePath = require('path').join(__dirname, 'frontend', 'public', 'style2', 'declaration-template.html');
  ...
  html = html.replace('__SIGNATORY_NAME__', esc(sigName));
  html = html.replace('__DIRIGEANT_1_NAME__ - __DIRIGEANT_1_ID__', esc(r0.name) + (r0.id ? ' - ' + esc(r0.id) : ''));
  html = html.replace('__DIRIGEANT_1_FUNCTION__', esc(r0.func || 'Administrateur'));
  html = html.replace('__DIRIGEANT_2_NAME__ - __DIRIGEANT_2_ID__', esc(r1.name) + (r1.id ? ' - ' + esc(r1.id) : ''));
  html = html.replace('__DIRIGEANT_2_FUNCTION__', esc(r1.func || ''));
  html = html.replace('__DIRIGEANT_3_NAME__ - __DIRIGEANT_3_ID__', esc(r2.name) + (r2.id ? ' - ' + esc(r2.id) : ''));
  html = html.replace('__DIRIGEANT_3_FUNCTION__', esc(r2.func || ''));
  html = html.replace('__FAIT_A__', esc(demande.fait_a || ''));
  html = html.replace('__DATE_ASSEMBLEE__', esc(demande.date_assemblee || ''));
  html += '\n<script>window.print();</script>\n';
  return html;
...
async function buildDossierDocument(documentKey, body) {
  ...
  if (documentKey === "attestation-identite") {
    const autoprint = String(body?.autoprint || "").toLowerCase() === "true" || body?.autoprint === 1;
    // Utiliser la mÃªme logique de fallback que les autres documents
    const lang = companyData?.lang_entre || "fr";
    const docLang = String(body?.file_language || lang).toLowerCase();
...
// â”€â”€ Legacy API helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
